...
async function checkImportMapConsistency() {
    ...
    if (!hasInconsistencies) {
        console.log(
            colorize('✅ All package versions are consistent across import maps!', 'green'),
        );
    }
    ...
    console.log(
        `  Packages with version conflicts: ${
            Array.from(packageVersions.values()).filter(vm => vm.size > 1).length
        }`,
    );
    ...
    if (packageVersions.size > 0) {
        ...
        const sortedPackages = Array.from(packageVersions.entries()).sort(([a], [b]) =>
            a.localeCompare(b)
        );
