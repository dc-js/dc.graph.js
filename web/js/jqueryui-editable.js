        ...
        init: function() {
            ...
            switch (this.options.autotext) {
                ...
                case 'auto':
                    ...
                    doAutotext = !$.trim(this.$element.text()).length && this.value !== null
                        && this.value !== undefined && !isValueByText;
                    break;
                default:
                    doAutotext = false;
            }
        ...
        setValue: function(value) {
            ...
            if (dt.isValid()) {
                ...
                $.each(values, function(k, v) {
                    ...
                    if (that['$'+k]) {
                        if (
                            k === 'minute' && that.options.minuteStep > 1 && that.options.roundTime
                        ) {
                            v = getNearest(that['$'+k], v);
                        }

                        if (
                            k === 'second' && that.options.secondStep > 1 && that.options.roundTime
                        ) {
                            v = getNearest(that['$'+k], v);
                        }
...
(function($) {
    ...
    $.extend($.fn.editableContainer.Popup.prototype, {
        ...
        innerShow: function() {
            this.call('open');
            var label = this.options.title || this.$element.data('ui-tooltip-title')
