...
export function matchOpposites(diagram, deleteProps, options) {
    ...
    diagram.cascade(100, true, multiplyProperties((e) => options.ease(e.deleting || 0), deleteProps, propertyInterpolate));
    ...
    function is_valid(sourcePort, targetPort) {
        return (_strategy.allowParallel() || !_wedges.some((e) => sourcePort.edges.indexOf(e) >= 0 && targetPort.edges.indexOf(e) >= 0)) && _strategy.isValid()(sourcePort, targetPort);
    }
    ...
    const _strategy = {
        isValid: property((sourcePort, targetPort) => 
            // draw_graphs is already enforcing this, but this makes more sense and i use xor any chance i get
             (diagram.portName.eval(sourcePort) === 'in') ^ (diagram.portName.eval(targetPort) === 'in')
        ),
        ...
        finishDragEdge(source, target) {
            if(is_valid(source.port, target.port)) {
                ...
                if(options.delete_edges) {
                    const edgeKeys = source.port.edges.map(diagram.edgeKey.eval).concat(target.port.edges.map(diagram.edgeKey.eval));
