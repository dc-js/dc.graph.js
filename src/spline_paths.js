...
export function splinePaths(pathreader, pathprops, hoverprops, selectprops, pathsgroup) {
    ...
    function path_keys(path, unique) {
        unique = unique !== false;
        const keys = pathreader.elementList.eval(path).filter((elem) => pathreader.elementType.eval(elem) === 'node').map((elem) => pathreader.nodeKey.eval(elem));
        return unique ? uniq(keys) : keys;
    }

    // check if entire path is present in this view
    function pathIsPresent(path) {
        return pathreader.elementList.eval(path).every((element) => pathreader.elementType.eval(element) !== 'node' ||
    ...
    // helper functions
    const vecDot = function(v0, v1) { return v0.x*v1.x+v0.y*v1.y; };
    const vecMag = function(v) { return Math.sqrt(v.x*v.x + v.y*v.y); };
    ...
    function drawSpline(paths) {
        ...
        edge
            .attr('stroke', (p) => selected.indexOf(p) !== -1 && selectprops.edgeStroke ||
                    hoverpaths.indexOf(p) !== -1 && hoverprops.edgeStroke ||
                    pathprops.edgeStroke || 'black')
            .attr('opacity', (p) => selected.indexOf(p) !== -1 && selectprops.edgeOpacity ||
    ...
    const _mode = mode('draw-spline-paths', {
        ...
        parent(p) {
            ...
            highlight_paths_group
                ...
                .on(`hover_changed.draw-spline-paths-${  _anchor}`, p ? (hpaths) => {
                    _hoverpaths = hpaths;
                    drawSpline(_paths);
                } : null);
        }
